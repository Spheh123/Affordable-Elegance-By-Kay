...
export async function getState() {
  const store = getStore({ name: "salon-booking-system", consistency: "strong" });
  return (await store.get(STORE_KEY, { type: "json" })) || defaultState;
}

export async function setState(state: unknown) {
  const store = getStore({ name: "salon-booking-system", consistency: "strong" });
